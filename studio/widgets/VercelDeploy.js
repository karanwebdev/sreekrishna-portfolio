...
export default function VercelDeploy({ deployHook, title }) {
  console.log('[VercelDeploy] Widget loaded with:', { deployHook, title });
  
  const [status, setStatus] = React.useState("idle");
  const [message, setMessage] = React.useState("");

  const handleDeploy = async () => {
    console.log('[VercelDeploy] Deploy button clicked');
    console.log('[VercelDeploy] Deploy hook URL:', deployHook);
    setStatus("deploying");
    setMessage("Triggering deploy...");

    try {
      const response = await fetch(deployHook, { method: "POST" });
      console.log('[VercelDeploy] Response received:', response.status, response.ok);

      if (response.ok) {
        const data = await response.json();
        console.log('[VercelDeploy] Deploy response:', data);
        setStatus("success");
        setMessage("Deploy triggered successfully! Check Vercel for progress.");
      } else {
        console.error('[VercelDeploy] Deploy failed:', response.status, response.statusText);
        setStatus("error");
  ...
  console.log('[VercelDeploy] Rendering widget...');
  
  return (
    <DashboardWidget header={title || "Vercel Deploy"}>
      <div style={{ padding: "1.5em" }}>
        <p style={{ marginBottom: "1em", color: "#666", fontSize: "14px" }}>
          Click the button below to manually trigger a production deployment to
          Vercel.
        </p>
